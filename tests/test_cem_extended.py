...
def test_cem_actions_short_list():
    ...
    best, _, _, _, _ = run_cem(_frame(), actions=actions, mock_scout_result=mock, use_scout=False)
    assert best in ("W", "A")
...
def test_cem_avoid_weight_zero():
    mock = ([0.5] * 10, 1.0, "")
    best, scores, _, _, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False, avoid_weight=0.0)
    assert scores[0] == 0.5


def test_cem_avoid_weight_one():
    mock = ([0.8] * 10, 0.5, "")
    best, scores, _, _, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False, avoid_weight=1.0)
...
def test_cem_last_actions_empty():
    ...
    mock = ([0.7] * 10, 0.0, "")
    best, _, _, _, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False, last_actions=[])
    assert best == "W"


def test_cem_last_objective_passed():
    mock = ([0.5] * 10, 0.0, "reach flag")
    _, _, _, obj, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False, last_objective="old")
...
def test_cem_scores_order():
    ...
    best, _, _, _, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False, last_actions=["W"])
...
def test_cem_mock_six_elements():
    ...
    _, _, _, obj, _ = run_cem(_frame(), mock_scout_result=mock[:3], use_scout=False)
    assert obj == "obj"


def test_cem_mock_two_elements_no_objectives():
    mock = ([0.5] * 10, 0.0)
    _, _, _, obj, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False)
    assert obj == ""


def test_cem_frame_shape_any():
    mock = ([0.5] * 10, 0.0, "")
    best, _, _, _, _ = run_cem(_frame(50, 50), mock_scout_result=mock, use_scout=False)
...
def test_cem_repeat_penalty_space_two():
    ...
    best, _, _, _, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False, last_actions=["space", "space"])
    assert best != "space"
...
def test_cem_repeat_penalty_three_w():
    ...
    best, _, _, _, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False, last_actions=["W", "W", "W"])
    assert best != "W"
...
def test_cem_avoid_applies_to_all():
    mock = ([0.5] * 10, 0.3, "")
    _, scores, _, _, _ = run_cem(_frame(), mock_scout_result=mock, use_scout=False, avoid_weight=1.0, last_actions=[])
    assert all(abs(s - 0.2) < 1e-5 for s in scores)


def test_cem_single_action_list_padded():
    mock = ([1.0], 0.0, "")
    best, scores, _, _, _ = run_cem(_frame(), actions=["W"], mock_scout_result=mock, use_scout=False)
